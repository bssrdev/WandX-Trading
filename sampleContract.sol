 ...
 */
contract TOKEN_NAMEToken is Ownable, MintableToken {
  ...
  // Token details
  string public constant name = "TOKEN_NAMEToken";
  ...
  // decimal places
  uint8 public constant decimals = DECIMAL_PLACES;
   ...
   */
  function TOKEN_NAMEToken(address _owner) {
      //Total of tokens
      totalSupply = TOTAL_TOKEN_SUPPLY * exponent;
      //mint(TEAM_ADDRESS, TEAM_SHARE * exponent);
      //mint(ADVISOR_ADDRESS, ADVISOR_SHARE * exponent);

      // tokens for Funders+advisors, PreSales
      // tokensMinted = tokensMinted.add(TEAM_SHARE * exponent);
      // balances[TEAM_ADDRESS] = TEAM_SHARE * exponent;
      // tokensMinted = tokensMinted.add(ADVISOR_SHARE * exponent);
      // balances[ADVISOR_ADDRESS] = ADVISOR_SHARE * exponent;
  }
...
/**
 * @title TOKEN_NAME CrowSale/ICO contract
 ...
 */
contract TOKEN_NAMECrowdsale is Ownable
{
    using SafeMath for uint256;

    // The token being sold
    TOKEN_NAMEToken public token;
     ...
     */
    function TOKEN_NAMECrowdsale() {
        wallet = msg.sender;
        token = new TOKEN_NAMEToken(msg.sender);
    }
    ...
    /**
      @dev proxy method for TOKEN_NAME Tokens batch transfers method, so that contract owner can call token methods
      @param _accounts buyers accounts that will receive the presale tokens
      @param _tokens   Amount of the tokens to be transferred to each account in _accounts list
      @return A boolean that indicates if the operation is successful.
     */
    ...
    /**
      @dev proxy method for TOKEN_NAME Tokens raiseInitialSupply method, so that contract owner can call token methods
      @param _supply delta number of tokens to be added to total supply
      @return A boolean that indicates if the operation is successful.
     */
    function raiseInitialSupply(uint256 _supply) onlyOwner public returns (bool) {
    ...
    /**
      @dev proxy method for TOKEN_NAME Tokens burn method, so that contract owner can call token methods
      @return A boolean that indicates if the operation is successful.
     */
