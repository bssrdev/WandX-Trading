 ...
 */
contract ExtraXToken is Ownable, MintableToken {
  ...
  // Token details
  string public constant name = "ExtraXToken";
   ...
   */
  function ExtraXToken(address _owner) {
      //Total of tokens
...
/**
 * @title ExtraX CrowSale/ICO contract
 ...
 */
contract ExtraXCrowdsale is Ownable
{
    using SafeMath for uint256;

    // The token being sold
    ExtraXToken public token;
     ...
     */
    function ExtraXCrowdsale() {
        wallet = msg.sender;
        token = new ExtraXToken(msg.sender);
    }
    ...
    /**
      @dev proxy method for ExtraX Tokens batch transfers method, so that contract owner can call token methods
      @param _accounts buyers accounts that will receive the presale tokens
      @param _tokens   Amount of the tokens to be transferred to each account in _accounts list
      @return A boolean that indicates if the operation is successful.
     */
    ...
    /**
      @dev proxy method for ExtraX Tokens raiseInitialSupply method, so that contract owner can call token methods
      @param _supply delta number of tokens to be added to total supply
      @return A boolean that indicates if the operation is successful.
     */
    function raiseInitialSupply(uint256 _supply) onlyOwner public returns (bool) {
    ...
    /**
      @dev proxy method for ExtraX Tokens burn method, so that contract owner can call token methods
      @return A boolean that indicates if the operation is successful.
     */
