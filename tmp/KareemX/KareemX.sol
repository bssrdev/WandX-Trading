 ...
 */
contract KareemXToken is Ownable, MintableToken {
  ...
  // Token details
  string public constant name = "KareemXToken";
  string public constant symbol = "KAREMX";
   ...
   */
  function KareemXToken(address _owner) {
      //Total of tokens
...
/**
 * @title KareemX CrowSale/ICO contract
 ...
 */
contract KareemXCrowdsale is Ownable
{
    using SafeMath for uint256;

    // The token being sold
    KareemXToken public token;
     ...
     */
    function KareemXCrowdsale() {
        wallet = msg.sender;
        token = new KareemXToken(msg.sender);
    }
    ...
    /**
      @dev proxy method for KareemX Tokens batch transfers method, so that contract owner can call token methods
      @param _accounts buyers accounts that will receive the presale tokens
      @param _tokens   Amount of the tokens to be transferred to each account in _accounts list
      @return A boolean that indicates if the operation is successful.
     */
    ...
    /**
      @dev proxy method for KareemX Tokens raiseInitialSupply method, so that contract owner can call token methods
      @param _supply delta number of tokens to be added to total supply
      @return A boolean that indicates if the operation is successful.
     */
    function raiseInitialSupply(uint256 _supply) onlyOwner public returns (bool) {
    ...
    /**
      @dev proxy method for KareemX Tokens burn method, so that contract owner can call token methods
      @return A boolean that indicates if the operation is successful.
     */
