 ...
 */
contract RTuTwo2019Token is Ownable, MintableToken {
  ...
  // Token details
  string public constant name = "RTuTwo2019Token";
   ...
   */
  function RTuTwo2019Token(address _owner) {
      //Total of tokens
...
/**
 * @title RTuTwo2019 CrowSale/ICO contract
 ...
 */
contract RTuTwo2019Crowdsale is Ownable
{
    using SafeMath for uint256;

    // The token being sold
    RTuTwo2019Token public token;
     ...
     */
    function RTuTwo2019Crowdsale() {
        wallet = msg.sender;
        token = new RTuTwo2019Token(msg.sender);
    }
    ...
    /**
      @dev proxy method for RTuTwo2019 Tokens batch transfers method, so that contract owner can call token methods
      @param _accounts buyers accounts that will receive the presale tokens
      @param _tokens   Amount of the tokens to be transferred to each account in _accounts list
      @return A boolean that indicates if the operation is successful.
     */
    ...
    /**
      @dev proxy method for RTuTwo2019 Tokens raiseInitialSupply method, so that contract owner can call token methods
      @param _supply delta number of tokens to be added to total supply
      @return A boolean that indicates if the operation is successful.
     */
    function raiseInitialSupply(uint256 _supply) onlyOwner public returns (bool) {
    ...
    /**
      @dev proxy method for RTuTwo2019 Tokens burn method, so that contract owner can call token methods
      @return A boolean that indicates if the operation is successful.
     */
